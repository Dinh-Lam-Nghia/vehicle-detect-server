...
def get_log_table(role: int):
    ...
    sql = ""
    if (role == 0):
        sql = f"SELECT vehicle.*, log.time, log.status, user.name FROM log INNER JOIN vehicle ON log.vehicle_id = vehicle.id INNER JOIN user ON vehicle.user_id = user.id WHERE log.role = 0 AND vehicle.active != 2"
    else:
...
def get_all_vehicle_admin(role: int):
    ...
    if (role == '0'):
        ...
        sql = f"SELECT vehicle_staff.* FROM log INNER JOIN vehicle_staff ON log.vehicle_id = vehicle_staff.id WHERE log.role = '{role}'"
    cursor.execute(sql)
    result = cursor.fetchall() 
    result = convert_to_dict(cursor, result)
    cursor.close()
...
def accpect_request(vehicle_id : str):
    conn = mysql.connector.connect(**db_config)
    cursor = conn.cursor()
    sql = f"UPDATE vehicle SET active = 0 WHERE vehicle_id = '{vehicle_id}'"
    cursor.execute(sql)
    conn.commit()

def remove_request(vehicle_id : str):
    conn = mysql.connector.connect(**db_config)
    cursor = conn.cursor()
